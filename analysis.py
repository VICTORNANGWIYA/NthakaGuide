...
def add_analysis():
    ...
    record = AnalysisHistory(
        ...
        # Farmer context — sent by recommend.py via farmerContext
        land_use      = data.get("land_use"),
        previous_crop = data.get("previous_crop") or None,

        # Soil inputs (handle both camelCase and snake_case from frontend)
        ...
        # Top recommendation (denormalised for fast listing)
...
def get_stats():
    ...
    # FIX: access .recommended_crop attribute, not positional index [0]
    # SQLAlchemy named-tuple rows support attribute access reliably across DBs
    top_crop_row = (
        db.session.query(
    ...
    return jsonify({
        ...
        # FIX: use attribute access instead of positional [0] indexing
        ...
        # FIX: pass include_full=False — stats only needs the summary, not all crops JSON
